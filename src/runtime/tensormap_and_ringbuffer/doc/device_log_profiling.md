...
Thread 0:   complete:    1485.020us (42.7%)  [notify: edges=432, max_degree=2, avg=1.2]
...
|-------|-------------|-------------|
| **complete** | Polls handshake on each managed core; when a core completes, traverses the task's fanout list via `pto2_scheduler_on_task_complete`, decrements consumer fanin counts, and enqueues newly ready consumers | `notify`: `edges` = total fanout edges traversed, `max_degree` = largest fanout list, `avg` = average fanout per completed task |
...
- **complete** is the second largest (~40-45%) because fanout traversal involves CAS-based fanin decrement and conditional ready-queue pushes.
